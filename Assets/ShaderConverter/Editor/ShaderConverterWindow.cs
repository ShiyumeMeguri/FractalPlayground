    ...
    public class ShaderConverterWindow : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            if (m_inputShaderType == ShaderLangType.ShaderToy && m_selectedTabIndex != 1)
            {
                ...
                m_iChannel0Type[m_selectedTabIndex] = EditorGUILayout.Popup(m_iChannel0Type[m_selectedTabIndex], m_textureTypeArray, GUILayout.MinWidth(20));
                EditorGUILayout.Space();
                EditorGUILayout.LabelField("iChannel1 Type:", GUILayout.Width(100));
                m_iChannel1Type[m_selectedTabIndex] = EditorGUILayout.Popup(m_iChannel1Type[m_selectedTabIndex], m_textureTypeArray, GUILayout.MinWidth(20));
                EditorGUILayout.Space();
                EditorGUILayout.LabelField("iChannel2 Type:", GUILayout.Width(100));
                m_iChannel2Type[m_selectedTabIndex] = EditorGUILayout.Popup(m_iChannel2Type[m_selectedTabIndex], m_textureTypeArray, GUILayout.MinWidth(20));
                EditorGUILayout.Space();
                EditorGUILayout.LabelField("iChannel3 Type:", GUILayout.Width(100));
                m_iChannel3Type[m_selectedTabIndex] = EditorGUILayout.Popup(m_iChannel3Type[m_selectedTabIndex], m_textureTypeArray, GUILayout.MinWidth(20));
                EditorGUILayout.EndHorizontal();
            }
            ...
            EditorGUILayout.LabelField("About Shader Converter: Version: 1.2");
        ...
        protected void Convert(int selectedTabIndex)
        {
            ...
            string outputShaderContent = Convert(inputShaderContent, shaderName, mainTexType, secondTexType, thirdTexType, fourthTexType);
            SetOutputShaderContent(selectedTabIndex, outputShaderContent);
        }
        
        protected string Convert(string inputShaderContent, string shaderName, int mainTexType, int secondTexType, int thirdTexType, int fourthTexType)
        {
            if (string.IsNullOrEmpty(inputShaderContent) || string.IsNullOrWhiteSpace(inputShaderContent))
            {
                Debug.LogWarning("Warning: Please input or paste working shader source code first.");
